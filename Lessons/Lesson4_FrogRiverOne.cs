...
namespace Codility.Lessons
{
    ...
    */
            int earliestTime = -1;
            Hashtable hshTime = new Hashtable();
            int numberOfLeaves = 0;

            for (int index = 0; index < A.Length; index++)
            {
                // Hashtable to maintain unique leaf poistion.
                if (!hshTime.ContainsKey(A[index]))
                {
                    hshTime.Add(A[index], index);

                    // Count Number leaves that falls between 1 and X
                    if (A[index] <= X)
                        numberOfLeaves++;
